...
| `docs/{writings,dataviz,visual-field-notes,blog,travels,physComp}/index.md` | Reserved section folders from the Atlas plan (see Part 6), created with a one-line "coming soon" stub — not wired into `mkdocs.yml`'s nav or any TSV yet, since there's no real content to surface. Get built out (and wired in) when actual content for each exists. |
